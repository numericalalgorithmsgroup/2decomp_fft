...
! This is the Fujitsu SSL implementation of the FFT library
! Note that the SSL engine used only supports transform size 
! with prime factor 2 and is suited for doing multiple repeated
! transforms. Other SSL engines are available.
  ...
  subroutine init_fft_engine
    ...
    if (KIND(0.0) == mytype) then
       ...
       call decomp_2d_abort(errorcode, &
            'Choosen SSL engine only suppport double-precision calculations')
    end if

    if (nrank==0) then
       write(*,*) ' '
       write(*,*) '***** Using the SSL engine *****'
       write(*,*) ' '
    end if
    ...
    allocate(scratch(2*cbuf_size + 70))  ! twice space for ssl library
...
#include "factor.f90"
  ...
  subroutine c2c_1m_x(inout, isign, decomp)
    ...
    isw = 1;
    if (x_goodsize) then
       call dvcfm1(buf,decomp%xsz(1),isw,isign,scratch,ierror)
    end if
    isw=0;
    do k=1,decomp%xsz(3)
       do j=1,decomp%xsz(2)
  ...
  subroutine c2c_1m_y(inout, isign, decomp)
    ...
    isw = 1;
    if (y_goodsize) then
       call dvcfm1(buf,decomp%ysz(2),isw,isign,scratch,ierror)
    end if
    isw = 0;
  ...
  subroutine c2c_1m_z(inout, isign, decomp)
    ...
    isw = 1;
    if (z_goodsize) then
       call dvcfm1(buf,decomp%zsz(3),isw,isign,scratch,ierror)
    end if
  ...
  subroutine r2c_1m_x(input, output)
    ...
    if (x_goodsize) then
       call dvcfm1(buf,s1,isw,isign,scratch,ierror)
    end if
    ...
    do k=1,s3
       do j=1,s2
          ...
          if (x_goodsize) then
             ...
             call spcfft(buf,s1,isign,scratch)
          end if
          ! note d1 ~ s1/2+1
  ...
  subroutine r2c_1m_z(input, output)
    ...
    if (z_goodsize) then
       call dvcfm1(buf,s3,isw,isign,scratch,ierror)
    end if
    ...
    do j=1,s2
       do i=1,s1
          ...
          if (z_goodsize) then
             ...
             call spcfft(buf,s3,isign,scratch)
          end if
          do k=1,d3
             output(i,j,k) = buf(k)
          end do
  ...
  subroutine c2r_1m_x(input, output)
    ...
    if (x_goodsize) then
       call dvcfm1(buf,d1,isw,isign,scratch,ierror)
    end if
    ...
    do k=1,d3
       do j=1,d2
          ...
          if (x_goodsize) then
             ...
             call spcfft(buf,d1,isign,scratch)
          end if
          do i=1,d1
  ...
  subroutine c2r_1m_z(input, output)
    ...
    if (z_goodsize) then
       call dvcfm1(buf,d3,isw,isign,scratch,ierror)
    end if
    ...
    do j=1,d2
       do i=1,d1
          ...
          if (z_goodsize) then
             call dvcfm1(buf,d2,isw,isign,scratch,ierror)
          else
             call spcfft(buf,d3,isign,scratch)
          end if
          do k=1,d3
